...
#' Test complicated scenario where there are merge, split, new cluster, code not filled the next day, etc. but no merging!
test_that("Not merging, complicated merge, split, empty code, new code", {
  ...
  meta_clusters_per_cell[['metacluster_4']] <- as.factor(sapply(1:num_codes_total, function(i) {
    if (i %in% c(1, 2, 33, 4, 39, 9, 15, 24, 25)) {
      return(1)
    } else if (i %in% c(6,26,27)) {
      return(2)
    } else if (i %in% c(34, 38, 7, 40, 23)) {
      return(3)
    } else if (i %in% c(21)) {
