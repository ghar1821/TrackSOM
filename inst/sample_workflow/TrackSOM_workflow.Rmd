...
For simplicity, we will use the synthetic dataset outlined in our manuscript downloadable from [bioArxiv](https://www.biorxiv.org/content/10.1101/2021.06.08.447468v1).
The dataset is stored under `inst` directory: [link](https://github.com/ghar1821/TrackSOM/tree/master/inst/extdata).

# Importing TrackSOM package

If you have not installed the TrackSOM package, please run the following chunk of code to install it using the `devtools` R package:
```{r}
...
TrackSOM relies on various packages to operate, and unfortunately some of them contain functions that are named the same. 
We are still in the process of tidying this up, but for now, please do not worry about this as it does not impact its functionality in any way.
...
Sometimes, it is possible to have the dataset stored as the `data.table` object files. 
...
The function also accept parameters that are built into [FlowSOM](https://bioconductor.org/packages/release/bioc/html/FlowSOM.html)'s `ReadInput`, `BuildSOM` and `BuildMST` functions.
...
* `ConcatenateClusteringDetails`: assuming your dataset is stored as a `data.table` object, the function attaches the meta-cluster ID and SOM nodes for each cell as separate columns.
...
To run `ConcatenateClusteringDetails`, you need to oass the following parameters:
...
head(dat.clust)
```
The function attach extra 3 columns:
...
To draw a network plot, we need to call the `DrawNetworkPlot` function.
